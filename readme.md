...
- login account (multiple or single?) just for volunteers so they can add photos of dogs
...
- get access to the municipal dog shelter database to get their data from there so there is no need to duplicate data (might be hard but it's talkable)
...
- new app with dogs/cats out and where they are (with photos and special guest account to allow owners to upload photos)
